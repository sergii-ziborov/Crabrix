...
extension WASIBridgeToHost {
    ...
    /// Register WASI imports while allowing a bounded embedder to interrupt
    /// immediately before every host call. The ordinary link API remains
    /// unchanged for clients that do not need an interruption hook.
    public func link(
