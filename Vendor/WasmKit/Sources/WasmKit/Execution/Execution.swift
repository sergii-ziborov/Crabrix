...
extension Execution {
    ...
    mutating func runTokenThreaded(sp: inout Sp, pc: inout Pc, md: inout Md, ms: inout Ms) throws {
            ...
            defer { stats.dump() }
        #endif
        let instructionLimiter = store.value.instructionLimiter
        let limiterBatchSize: UInt64 = 4_096
        var instructionsUntilLimiterCheck = limiterBatchSize
        var opcode = pc.read(OpcodeID.self)
        while true {
            do {
                while true {
                    if let instructionLimiter {
                        instructionsUntilLimiterCheck -= 1
                        if instructionsUntilLimiterCheck == 0 {
                            try instructionLimiter.consume(instructionCount: limiterBatchSize)
                            instructionsUntilLimiterCheck = limiterBatchSize
                        }
                    }
                    #if EngineStats
                        stats.track(inst)
