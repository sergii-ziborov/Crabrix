...
enum AlgorithmCourseData {
    ...
    private static let hashing = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("complement-lookup", "Complement Lookup", .easy, "For each number, ask whether target - number has already appeared.", "pair sums, balanced deltas, matching requirements", "O(n) expected time · O(n) space", "Return the zero-based indexes of two values summing to 13.", "[6, 4, 9, 7] target=13", "1,2"),
    ...
    private static let ranges = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("imos-sweep", "Imos Sweep", .medium, "Record interval starts and ends, then sweep a running active count.", "coverage, bookings, overlapping demand", "O((n+q)) on discrete coordinates · O(n) space", "Return the maximum simultaneous load from inclusive updates (0,2,+2), (1,3,+3), (3,4,+1).", "n=5", "4"),
            p("fenwick-tree", "Fenwick Tree", .hard, "Store partial sums in power-of-two ranges and move indexes by their lowest set bit.", "dynamic prefix sums, frequency tables, inversion counts", "O(log n) update/query · O(n) space", "After adding 5 at index 1, 2 at 3, and 4 at 4, return prefix sum through index 3.", "1-indexed updates", "7"),
            p("segment-tree", "Segment Tree", .hard, "Represent intervals as a binary tree so a query visits only canonical covering nodes.", "dynamic range min/max/sum and interval aggregation", "O(log n) update/query · O(n) space", "Build range minimum, change index 2 to 0, then query inclusive range 1..3.", "[5,4,7,2]", "0"),
        ]
    )
    ...
    private static let stacks = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("queue-with-stacks", "Queue from Two Stacks", .medium, "Reverse the input stack into an output stack only when the output side is empty.", "amortised FIFO from LIFO primitives", "O(1) amortised operation · O(n) space", "Enqueue 3, 5, 8; dequeue twice; enqueue 2; return the queue front.", "operations", "8"),
    ...
    private static let heaps = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("lazy-heap-deletion", "Lazy Heap Deletion", .hard, "Record invalidated values separately and discard stale roots only when they surface.", "sliding heaps and priority updates without handles", "O(log n) amortised operation · O(n) space", "Push 4,7,2, lazily delete 2, then return the valid minimum.", "operations", "4"),
    ...
    private static let strings = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("trie", "Prefix Trie", .medium, "Store one edge per next symbol so shared prefixes occupy shared nodes.", "prefix lookup, autocomplete, dictionary traversal", "O(total symbols) build · O(length) query", "Insert crab, crate, rust; return count of words starting with cr.", "words", "2"),
    ...
    private static let mathBits = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("bitmask-enumeration", "Bitmask Enumeration", .easy, "Use each bit as an include/exclude choice for one item.", "small subsets, permissions, feature combinations", "O(2^n*n) time · O(1) state excluding output", "Return masks whose selected values sum to 5.", "values=[1,2,4]", "3,5"),
    ...
    private static let advanced = AlgorithmCategorySeed(
        ...
        patterns: [
            ...
            p("disjoint-set-union", "Disjoint Set Union", .medium, "Compress find paths and attach smaller trees beneath larger roots.", "dynamic connectivity, Kruskal, grouping", "Almost O(1) amortised · O(n) space", "Union (0,1), (1,2), (3,4); return component count for n=5.", "operations", "2"),
            ...
            p("treap", "Randomized Treap", .hard, "Keep BST order by key and heap order by random priority; split and merge preserve both.", "ordered sets with expected logarithmic operations", "O(log n) expected operation · O(n) space", "Insert 5,2,8,1 and return inorder keys after deleting 2.", "operations", "1,5,8"),
            ...
            p("dsu-rollback", "Rollback DSU", .hard, "Record every parent or size mutation so recursion over time can undo unions without path compression.", "offline dynamic connectivity and divide-over-time", "O(log n) union/find typical · O(changes) history", "After unions (0,1),(1,2), checkpoint, union (2,3), rollback, return whether 0 and 3 connect.", "n=4", "false"),
